...
struct Config {
    explicit Config()
        : id(0), election_tick(0), heartbeat_tick(0), applied(0), max_size_per_msg(0), max_committed_size_per_ready(0),
          max_uncommitted_entries_size(0), max_inflight_msgs(0), check_quorum(false), pre_vote(false),
          read_only_option(ReadOnlySafe), disable_proposal_forwarding(false) {}
    ...
    // from previous configuration will panic if peers is set. peer is private
    // and only used for testing right now.
    ...
    //
    //              election_tick = 10 * heartbeat_tick
    //
    ...
    // MaxInflightMsgs to avoid overflowing that sending buffer.
    ...
    // it should (clock can move backward/pause without any bound). read_index
    ...
    // hybrid logical clock from assigning the timestamp and then forwarding
    // the data to the leader.
