...
// Raft server's implementation.
// The implementation uses a variant of state design pattern, in which the state
// of the raft server is determined by two class members: TICK_ and STEP_.
class Raft {
public:
    ...
    // Invoke the TICK_ function.
    void tick();

    // Change TICK_ and STEP_, but does not changes TERM_ and VOTE_.
    ...
    // CAMPAIGN_TYPE represents the type of campaigning the reason we use the
    // type of string instead of uint64 is because it's simpler to compare and
    // fill in raft entries.
    void campaign(const std::string &campaign_type);

    // Collects the number of vote to the
    uint32_t poll(uint64_t id, proto::MessageType type, bool v);

    // Perform an action according to the receiving procedure call MSG.
    virtual Status step(proto::MessagePtr msg);

    Status step_leader(proto::MessagePtr msg);

    // step_candidate is shared by StateCandidate and StatePreCandidate; the
    ...
    // Get the quorum of the current cluster.
    //
    // A QUORUM is the minimum number of votes that a distributed transcation
    // has to obtain in order to be allowed to be updated to the node's state
    // machine.
    ...
    // Returns true if a message was sent. The sendIfEmpty argument controls
    // whether messages with no entries will be sent ("empty" messages are
    // useful to convey updated Commit indexes, but are undesirable when we're
    ...
    // Run by followers and candidates after ElectionTimeout.
    void tick_election();

    // Run by the leader to maintain its leadership within the cluster.
    void tick_heartbeat();
    ...
    // Check if there are quorum number of active peers.
    ...
    // false. If not, the increase in uncommitted entry size is recorded and
    // the method returns true.
    bool increase_uncommitted_size(const std::vector<proto::EntryPtr> &entries);

    // reduce_uncommitted_size accounts for the newly committed entries by
...
public:
    ...
    // lead_transferee_ is id of the leader transfer target when its value is
    ...
    // a time. This is enforced via pending_conf_index_, which is set to a
    // value >= the log index of the latest pending configuration
    // change (if any). Config changes are only allowed to be proposed if the
    // leader's applied index is greater than this value.
    ...
    // The internal clock used for the raft server. Has two variants:
    // - tick_election():
    // - tick_heartbeat():
    std::function<void()> tick_;

    // The transition function used to update the server's state. Has three
    // variants:
    // - step_follower()
    // - step_candidate()
    // - step_leader()
    ...
    // Device used to generate a random number as the duration for election
    // timeout.
