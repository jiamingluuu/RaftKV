...
public:
    explicit InFlights(uint64_t max_inflight_msgs)
        : start(0), count(0), size(static_cast<uint32_t>(max_inflight_msgs)) {}
...
public:
    explicit Progress(uint64_t max_inflight)
        : match(0), next(0), state(ProgressState::ProgressStateProbe), paused(false), pending_snapshot(0),
          recent_active(false), inflights(new InFlights(max_inflight)), is_learner(false) {}
