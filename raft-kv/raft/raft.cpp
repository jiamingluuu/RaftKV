...
Raft::Raft(const Config &c)
    : id_(c.id), term_(0), vote_(0), max_msg_size_(c.max_size_per_msg),
      ...
      state_(RaftState::Follower), is_learner_(false), lead_(0), lead_transferee_(0), pending_conf_index_(0),
      uncommitted_size_(0), read_only_(new ReadOnly(c.read_only_option)), election_elapsed_(0), heartbeat_elapsed_(0),
      check_quorum_(c.check_quorum), pre_vote_(c.pre_vote), heartbeat_timeout_(c.heartbeat_tick),
      election_timeout_(c.election_tick), randomized_election_timeout_(0),
      disable_proposal_forwarding_(c.disable_proposal_forwarding), random_device_(0, c.election_tick) {
    ...
    LOG_INFO("raft %lu [peers: [%s], term: %lu, commit: %lu, applied: %lu, last_index: %lu, last_term: %lu]", id_,
             node_str.c_str(), term_, raft_log_->committed_, raft_log_->applied_, raft_log_->last_index(),
             raft_log_->last_term());
}
...
void Raft::campaign(const std::string &campaign_type) {
    ...
    for (auto it = prs_.begin(); it != prs_.end(); ++it) {
        ...
        LOG_INFO("%lu [log_term: %lu, index: %lu] sent %s request to %lu at term %lu", id_, raft_log_->last_term(),
                 raft_log_->last_index(), proto::msg_type_to_string(vote_msg), it->first, term_);
...
uint32_t Raft::poll(uint64_t id, proto::MessageType type, bool v) {
    ...
    if (it == votes_.end()) {
        // Vote for the node itself.
        votes_[id] = v;
    }

    // Collects the vote from other nodes.
...
Status Raft::step(proto::MessagePtr msg) {
    if (msg->term == 0) {
        ...
        if (msg->type == proto::MsgVote || msg->type == proto::MsgPreVote) {
            ...
            if (!force && in_lease) {
                ...
                LOG_INFO(
                    "%lu [log_term: %lu, index: %lu, vote: %lu] ignored %s from %lu [log_term: %lu, index: %lu] "
                    "at term %lu: lease is not expired (remaining ticks: %d)",
                    id_, raft_log_->last_term(), raft_log_->last_index(), vote_, proto::msg_type_to_string(msg->type),
                    msg->from, msg->log_term, msg->index, term_, election_timeout_ - election_elapsed_);
                return Status::ok();
            }
        }
        switch (msg->type) {
        ...
        default:
            LOG_INFO("%lu [term: %lu] received a %s message with higher term from %lu [term: %lu]", id_, term_,
                     proto::msg_type_to_string(msg->type), msg->from, msg->term);
        ...
        if ((check_quorum_ || pre_vote_) && (msg->type == proto::MsgHeartbeat || msg->type == proto::MsgApp)) {
            ...
            LOG_INFO(
                "%lu [log_term: %lu, index: %lu, vote: %lu] rejected %s from %lu [log_term: %lu, index: %lu] at "
                "term %lu",
                id_, raft_log_->last_term(), raft_log_->last_index(), vote_, proto::msg_type_to_string(msg->type),
                msg->from, msg->log_term, msg->index, term_);
            proto::MessagePtr m(new proto::Message());
            m->to = msg->from;
            ...
            // ignore other cases
            LOG_INFO("%lu [term: %lu] ignored a %s message with lower term from %lu [term: %lu]", id_, term_,
                     proto::msg_type_to_string(msg->type), msg->from, msg->term);
        }
        return Status::ok();
    }
    ...
    case proto::MsgHup: {
        if (state_ != RaftState::Leader) {
            ...
            Status status =
                raft_log_->slice(raft_log_->applied_ + 1, raft_log_->committed_ + 1, RaftLog::unlimited(), entries);
            if (!status.is_ok()) {
            ...
            if (pending > 0 && raft_log_->committed_ > raft_log_->applied_) {
                LOG_WARN(
                    "%lu cannot campaign at term %lu since there are still %u pending configuration canges to apply",
                    id_, term_, pending);
                return Status::ok();
            }
    ...
    case proto::MsgPreVote: {
        if (is_learner_) {
            ...
            LOG_INFO(
                "%lu [log_term: %lu, index: %lu, vote: %lu] ignored %s from %lu [log_term: %lu, index: %lu] at "
                "term %lu: learner can not vote",
                id_, raft_log_->last_term(), raft_log_->last_index(), vote_, proto::msg_type_to_string(msg->type),
                msg->from, msg->log_term, msg->index, msg->term);
            return Status::ok();
        }
        ...
        if (can_vote && this->raft_log_->is_up_to_date(msg->index, msg->log_term)) {
            LOG_INFO("%lu [log_term: %lu, index: %lu, vote: %lu] cast %s for %lu [log_term: %lu, index: %lu] at "
                     "term %lu",
                     id_, raft_log_->last_term(), raft_log_->last_index(), vote_, proto::msg_type_to_string(msg->type),
                     msg->from, msg->log_term, msg->index, term_);
...
Status Raft::step_leader(proto::MessagePtr msg) {
    ...
    // Step down if failed to check quorum.
    ...
    case proto::MsgProp: {
        ...
        if (lead_transferee_ != 0) {
            LOG_DEBUG("%lu [term %lu] transfer leadership to %lu is in progress; dropping proposal", id_, term_,
                      lead_transferee_);
            return Status::invalid_argument("raft proposal dropped");
        }
            ...
            if (e.type == proto::EntryConfChange) {
                if (pending_conf_index_ > raft_log_->applied_) {
                    LOG_INFO("propose conf %s ignored since pending unapplied configuration [index %lu, applied %lu]",
                             proto::entry_type_to_string(e.type), pending_conf_index_, raft_log_->applied_);
    ...
    case proto::MsgAppResp: {
        ...
        if (msg->reject) {
            LOG_DEBUG("%lu received msgApp rejection(last_index: %lu) from %lu for index %lu", id_,
                      msg->reject_hint, msg->from, msg->index);
            ...
            if (pr->maybe_update(msg->index)) {
                if (pr->state == ProgressStateProbe) {
                    ...
                    LOG_DEBUG("%lu snapshot aborted, resumed sending replication messages to %lu [%s]", id_,
    ...
    case proto::MsgSnapStatus: {
        ...
        if (!msg->reject) {
            pr->become_probe();
            LOG_DEBUG("%lu snapshot succeeded, resumed sending replication messages to %lu [%s]", id_, msg->from,
                      pr->string().c_str());
        } else {
            pr->snapshot_failure();
            pr->become_probe();
            LOG_DEBUG("%lu snapshot failed, resumed sending replication messages to %lu [%s]", id_, msg->from,
                      pr->string().c_str());
        }
    ...
    case proto::MsgUnreachable: {
        ...
        LOG_DEBUG("%lu failed to send message to %lu because it is unreachable [%s]", id_, msg->from,
                  pr->string().c_str());
        break;
    }
    case proto::MsgTransferLeader: {
        ...
        if (pr->match == raft_log_->last_index()) {
            ...
            LOG_INFO("%lu sends MsgTimeoutNow to %lu immediately as %lu already has up-to-date log", id_,
...
Status Raft::step_candidate(proto::MessagePtr msg) {
    ...
    case proto::MsgVoteResp: {
        ...
        LOG_INFO("%lu [quorum:%u] has received %lu %s votes and %lu vote rejections", id_, quorum(), gr,
                 proto::msg_type_to_string(msg->type), votes_.size() - gr);
        if (quorum() == gr) {
    ...
    case proto::MsgTimeoutNow: {
        LOG_DEBUG("%lu [term %lu state %d] ignored MsgTimeoutNow from %lu", id_, term_, state_, msg->from);
    }
    }
    return Status::ok();
}
...
Status Raft::step_follower(proto::MessagePtr msg) {
    ...
    case proto::MsgTimeoutNow:
        if (promotable()) {
            LOG_INFO("%lu [term %lu] received MsgTimeoutNow from %lu and starts an election to get leadership.", id_,
                     term_, msg->from);
    ...
    case proto::MsgReadIndexResp:
        if (msg->entries.size() != 1) {
            LOG_ERROR("%lu invalid format of MsgReadIndexResp from %lu, entries count: %lu", id_, msg->from,
                      msg->entries.size());
            return Status::ok();
        }
        ReadState rs;
...
void Raft::handle_append_entries(proto::MessagePtr msg) {
    ...
    if (ok) {
        ...
        LOG_DEBUG("%lu [log_term: %lu, index: %lu] rejected msgApp [log_term: %lu, index: %lu] from %lu", id_,
                  term, msg->index, msg->log_term, msg->index, msg->from)
...
void Raft::handle_snapshot(proto::MessagePtr msg) {
    ...
    if (restore(msg->snapshot)) {
        LOG_INFO("%lu [commit: %lu] restored snapshot [index: %lu, term: %lu]", id_, raft_log_->committed_, sindex,
                 sterm);
        proto::MessagePtr m(new proto::Message());
        m->to = msg->from;
        m->type = proto::MsgAppResp;
        ...
        LOG_INFO("%lu [commit: %lu] ignored snapshot [index: %lu, term: %lu]", id_, raft_log_->committed_, sindex,
                 sterm);
        proto::MessagePtr m(new proto::Message());
        m->to = msg->from;
        m->type = proto::MsgAppResp;
...
bool Raft::restore(const proto::Snapshot &s) {
    ...
    if (raft_log_->match_term(s.metadata.index, s.metadata.term)) {
        LOG_INFO(
            "%lu [commit: %lu, last_index: %lu, last_term: %lu] fast-forwarded commit to snapshot [index: %lu, "
            "term: %lu]",
            id_, raft_log_->committed_, raft_log_->last_index(), raft_log_->last_term(), s.metadata.index,
            s.metadata.term);
    ...
    if (!is_learner_) {
        for (uint64_t id : s.metadata.conf_state.learners) {
            if (id == id_) {
                LOG_ERROR("%lu can't become learner when restores snapshot [index: %lu, term: %lu]", id_,
    ...
    LOG_INFO(
        "%lu [commit: %lu, last_index: %lu, last_term: %lu] starts to restore snapshot [index: %lu, term: %lu]",
        id_, raft_log_->committed_, raft_log_->last_index(), raft_log_->last_term(), s.metadata.index, s.metadata.term);
...
SoftStatePtr Raft::soft_state() const { return std::make_shared<SoftState>(lead_, state_); }
...
void Raft::load_state(const proto::HardState &state) {
    if (state.commit < raft_log_->committed_ || state.commit > raft_log_->last_index()) {
        LOG_FATAL("%lu state.commit %lu is out of range [%lu, %lu]", id_, state.commit, raft_log_->committed_,
...
void Raft::send_append(uint64_t to) { maybe_send_append(to, true); }
...
bool Raft::maybe_send_append(uint64_t to, bool send_if_empty) {
    ...
    if (!status_term.is_ok() || !status_entries.is_ok()) { // send snapshot if we failed to get term or entries
        ...
        LOG_DEBUG("%lu [first_index: %lu, commit: %lu] sent snapshot[index: %lu, term: %lu] to %lu [%s]", id_,
                  raft_log_->first_index(), raft_log_->committed_, sindex, sterm, to, pr->string().c_str());
...
void Raft::send_heartbeat(uint64_t to, std::vector<uint8_t> ctx) {
    ...
    // When the leader sends out heartbeat message, the receiver(follower) might
    // not be matched with the leader or it might not have all the committed
...
void Raft::add_node(uint64_t id) { add_node_or_learner(id, false); }
...
bool Raft::append_entry(const std::vector<proto::Entry> &entries) {
    ...
    if (!increase_uncommitted_size(ents)) {
        LOG_DEBUG("%lu appending new entries to log would exceed uncommitted entry size limit; dropping proposal",
                  id_);
        // Drop the proposal.
        return false;
    }
...
void Raft::tick_heartbeat() {
    ...
    if (election_elapsed_ >= election_timeout_) {
        election_elapsed_ = 0;
        // step down if failed for checking quorum, indicating the current
        // leader is stale.
        if (check_quorum_) {
            proto::MessagePtr msg(new proto::Message());
            msg->from = id_;
...
void Raft::abort_leader_transfer() { lead_transferee_ = 0; }
