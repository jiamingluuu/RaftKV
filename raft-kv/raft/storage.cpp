...
Status MemoryStorage::entries(uint64_t low, uint64_t high, uint64_t max_size, std::vector<proto::EntryPtr> &entries) {
    assert(low < high);
    std::lock_guard<std::mutex> guard(mutex_);
...
Status MemoryStorage::create_snapshot(uint64_t index, proto::ConfStatePtr cs, std::vector<uint8_t> data,
