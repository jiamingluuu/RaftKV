...
struct ConfState {
    bool operator==(const ConfState &cs) const { return nodes == cs.nodes && learners == cs.learners; }
...
struct Message {
    Message()
        : type(MsgHup), to(0), from(0), term(0), log_term(0), index(0), commit(0), reject(false), reject_hint(0) {}

    bool operator==(const Message &msg) const {
        return type == msg.type && to == msg.to && from == msg.from && term == msg.term && log_term == msg.log_term &&
               index == msg.index && entries == msg.entries && commit == msg.commit && snapshot.equal(msg.snapshot) &&
               reject == msg.reject && reject_hint == msg.reject_hint && context == msg.context;
    }
...
struct HardState {
    ...
    bool equal(const HardState &hs) const { return term == hs.term && vote == hs.vote && commit == hs.commit; }
