...
Node *Node::start_node(const Config &conf, const std::vector<PeerContext> &peers) { return new RawNode(conf, peers); }

Node *Node::restart_node(const Config &conf) { return new RawNode(conf); }
