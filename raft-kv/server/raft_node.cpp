...
RaftNode::RaftNode(uint64_t id, const std::string &cluster, uint16_t port)
    : port_(port), pthread_id_(0), timer_(io_service_), id_(id), last_index_(0), conf_state_(new proto::ConfState()),
      snapshot_index_(0), applied_index_(0), storage_(new MemoryStorage()), snap_count_(defaultSnapCount) {
...
void RaftNode::maybe_trigger_snapshot() {
    ...
    LOG_DEBUG("start snapshot [applied index: %lu | last snapshot index: %lu], snapshot count[%lu]", applied_index_,
              snapshot_index_, snap_count_);

    std::promise<SnapshotDataPtr> promise;
    std::future<SnapshotDataPtr> future = promise.get_future();
    redis_server_->get_snapshot(std::move([&promise](const SnapshotDataPtr &data) { promise.set_value(data); }));
...
void RaftNode::report_snapshot(uint64_t id, SnapshotStatus status) { LOG_DEBUG("no impl yet"); }
