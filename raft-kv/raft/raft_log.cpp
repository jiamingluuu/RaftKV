    ...
    for (const proto::EntryPtr &entry : entries) {
        if (!match_term(entry->index, entry->term)) {
            if (entry->index < last_index()) {
                ...
                LOG_INFO("found conflict at index %lu [existing term: %lu, conflicting term: %lu], %s", entry->index,
                         t, entry->term, status.to_string().c_str());
            }
            return entry->index;
        }
    }
...
void RaftLog::commit_to(uint64_t to_commit) {
    ...
    if (committed_ < to_commit) {
        if (last_index() < to_commit) {
            LOG_FATAL(
                "to_commit(%lu) is out of range [lastIndex(%lu)]. Was the raft log corrupted, truncated, or lost?",
